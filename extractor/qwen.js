...
const OLLAMA_TIMEOUT_MS     = parseInt(process.env.OLLAMA_TIMEOUT_MS || '60000'); // BUG-008: was 5000ms, too short for slow machines
const OLLAMA_FALLBACK_URL   = process.env.OLLAMA_FALLBACK_URL   || 'http://localhost:11434';
const OLLAMA_FALLBACK_MODEL = process.env.OLLAMA_FALLBACK_MODEL || OLLAMA_MODEL;

/**
 * Call Ollama API with fast timeout.
 * Returns { response, source } on success, throws on failure.
 */
async function callOllama(url, model, prompt, source) {
  ...
  try {
    ...
    if (respText) {
      ...
      if (!jsonText) {
        ...
        if (repaired) return { parsed: repaired, source: source + '+repaired' };
      ...
      return { parsed, source: jsonText !== stripped ? source : source + '+extracted' };
    }
    ...
    if (thinkingText) {
      ...
      if (directParse) {
        ...
        if (typeof directParse === 'object' && typeof directParse.output === 'string') {
          ...
          if (embeddedParsed) return { parsed: embeddedParsed, source };
        }
        return { parsed: directParse, source };
      }
 ...
 * Tries primary Ollama → fallback Ollama → regex degraded mode.
...
JSON output:`;
  ...
  // 1. Try primary Ollama host
  try {
    const result = await callOllama(OLLAMA_URL, OLLAMA_MODEL, prompt, 'primary');
    parsed = result.parsed;
    source = result.source;
    console.log(`[extractor] used primary for ${url}`);
  } catch (primaryErr) {
    console.warn(`[extractor] primary failed for ${url}: ${primaryErr.message}`);

    // 2. Try fallback Ollama host
    try {
      const result = await callOllama(OLLAMA_FALLBACK_URL, OLLAMA_FALLBACK_MODEL, prompt, 'fallback');
      parsed = result.parsed;
      source = result.source;
