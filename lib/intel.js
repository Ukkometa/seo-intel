...
import { pageKey, canonicalizePages, isNavigable } from './url.js';

const __dirname = dirname(fileURLToPath(import.meta.url));
const VERSION = JSON.parse(readFileSync(join(__dirname, '..', 'package.json'), 'utf8')).version;

export const INTEL_SLICES = ['raw', 'audit', 'blog', 'competitor', 'graph'];
// Own-site slices are free; only the competitor slice (data the agent can't
// gather on its own) requires Solo.
export const FREE_SLICES = ['raw', 'audit', 'blog', 'graph'];
...
export function getIntel(db, project, opts = {}) {
  ...
  if (slice === 'graph')      envelope.data = collectGraph(db, project);
...
// Internal-link graph of YOUR site (target + owned), each node carrying its
// latest citability score — lean payload for force-directed rendering
// (Hermes cockpit, or any harness that wants structure-level reasoning:
// orphans, hubs, weak clusters). Same URL-resolution rules as the HTML
// generator in reports/generate-site-graph.js.
function collectGraph(db, project) {
  const nodes = db.prepare(`
    SELECT p.id, p.url, p.title, p.click_depth AS depth, d.role, c.score, c.tier
    FROM pages p
    JOIN domains d ON d.id = p.domain_id
    LEFT JOIN citability_scores c ON c.page_id = p.id
    WHERE d.project = ? AND d.role IN ('target', 'owned')
      AND (p.status_code IS NULL OR p.status_code = 200)
    ORDER BY p.click_depth ASC, p.id ASC
    LIMIT 600
  `).all(project);

  // Collapse www / index.html / #anchor variants of the same page into one
  // node before building the graph — see canonicalizePages for why.
  const { merged, idToCanonical, keyToId: urlToId } = canonicalizePages(nodes, (winner, loser) => {
    // Rescue signal off the row being discarded rather than losing it.
    if (winner.score == null && loser.score != null) {
      winner.score = loser.score;
      winner.tier = loser.tier;
    }
    if (!winner.title && loser.title) winner.title = loser.title;
  });

  const rawEdges = db.prepare(`
    SELECT l.source_id, l.target_url
    FROM links l
    JOIN pages p_src ON p_src.id = l.source_id
    JOIN domains d ON d.id = p_src.domain_id
    WHERE l.is_internal = 1 AND d.project = ? AND d.role IN ('target', 'owned')
  `).all(project);
  ...
  for (const e of rawEdges) {
    ...
    const targetId = urlToId.get(pageKey(e.target_url));
    // After canonicalization a nav link like `/#pricing` resolves back to the
    // page it sits on. That is a self-link: real, but not inbound authority.
    if (!targetId || targetId === sourceNode.id) continue;
    const key = `${sourceNode.id}-${targetId}`;
    if (seen.has(key)) continue;
    seen.add(key);
    edges.push({ source: sourceNode.id, target: targetId });
    inbound.set(targetId, (inbound.get(targetId) || 0) + 1);
  }

  for (const n of merged) n.inbound = inbound.get(n.id) || 0;
  const scored = merged.filter((n) => n.score != null);

  // Orphan detection is only meaningful when link data exists. A project
  // crawled before links were recorded (or with link extraction off) has zero
  // edges, which would make every page look orphaned and hand the user a list
  // of hundreds of non-problems. Report the missing input instead.
  const hasLinkData = rawEdges.length > 0;

  // Orphans = a navigable page with no internal inbound links, and not the
  // root. Machine-readable endpoints (llms.txt, skill.md, feeds, sitemaps) are
  // excluded: nothing is supposed to link to them, so reporting them as
  // orphans is a fix instruction the user should ignore.
  const orphans = hasLinkData
    ? merged
      .filter((n) => n.inbound === 0 && n.depth !== 0 && isNavigable(n.url))
      .map((n) => n.url)
    : [];
  ...
  return {
    ...
    summary: {
      pages: merged.length,
      links: edges.length,
      orphans: hasLinkData ? orphans.length : null,
      avg_citability: scored.length ? Math.round((scored.reduce((s, n) => s + n.score, 0) / scored.length) * 10) / 10 : null,
    },
    orphan_urls: orphans.slice(0, 50),
    note: hasLinkData
      ? 'Internal-link graph of your own site (raw crawl truth). Node score = latest AI citability; inbound = distinct pages linking here; orphans have none. www/index.html/#anchor variants of one page are merged into a single node.'
      : 'No internal link data stored for this project, so edges and orphan detection are unavailable — re-crawl to populate them. Nodes and citability scores below are still accurate.',
  };
}
